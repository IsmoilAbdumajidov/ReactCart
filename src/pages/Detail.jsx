            ...
            <div className='flex border overflow-hidden  flex-col sm:flex-row gap-3 col-span-1 rounded-md'>
                <div className='flex flex-wrap  sm:flex-nowrap  sm:overflow-y-scroll detail_img   gap-3 sm:flex-col sm:w-28 h-full'>
                    {state.detail[0]?.images.map((img, index) => (
                        <img className='cursor-pointer w-28 sm:w-full h-auto object-cover object-center' onClick={imgHanler} key={index} src={img} alt="" />
                    ))}
                </div>
                <div className='relative cursor-pointer'>
                    <div className='absolute bottom-0 rounded left-0 text-sm bg-green-600 text-white px-1'>{state.detail[0]?.type}</div>
                    <img ref={imgRef} className='w-full h-full object-cover object-center' src={state.detail[0]?.images[0]} alt="" />
            ...
            <div className="p-4 relative flex flex-col col-span-1  gap-2 flex-1 border rounded-md">
                ...
                <div className='flex flex-col sm:flex-row mt-auto gap-2'>
                    <div className='flex  flex-1'>
                        <button id='cart' onClick={() => quantityHandler(true, state.detail[0])} className='bg-green-500 gap-2 flex items-center justify-center transition-all hover:bg-green-600 hover:shadow-md  flex-1 text-white rounded-md py-2 hover:shadow-green-600'>
                    ...
                    <div className='flex gap-2'>
                        <button id='scale' onClick={() => toggleHandler('scale', 'UPDATE_SCALE', state.detail[0])} className='bg-amber-400 flex-1 flex justify-center hover:bg-amber-500 transition-all text-white rounded-md p-2'>
                        ...
                        <button id='wishlist' onClick={() => toggleHandler('wishlist', 'UPDATE_WISHLIST', state.detail[0])} className='bg-red-500 flex-1 flex justify-center hover:bg-red-600 transition-all text-white rounded-md p-2'>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
