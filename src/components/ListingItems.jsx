                ...
                {list.length ? list.map((card, index) => (
                    <div className='overflow-hidden flex flex-col shadow rounded-md hover:shadow-lg transition-all relative' key={index}>
                        <div className='relative cursor-pointer group'>
                            <div id='wishlist' onClick={() => toggleHandler('wishlist', 'UPDATE_WISHLIST', card)} className='absolute right-2 bg-white/20 top-2 w-10 h-10 rounded-full flex items-center justify-center'>
                        ...
                        <div className="p-4 relative flex flex-col  gap-2 flex-1 border">
                            ...
                            <div className='flex mt-auto gap-2'>
                                <button id='cart' onClick={() => toggleHandler('cart', 'UPDATE_CART', card)} className='bg-green-500 gap-2 flex items-center justify-center transition-all hover:bg-green-600 hover:shadow-md text-white rounded-md py-2 flex-1 hover:shadow-green-600'>
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
